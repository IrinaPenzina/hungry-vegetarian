...
const useRecipeStore = Pinia.defineStore('counter', {
  state() {
    return {
      searchQuery: '',
      tabs: ["Breakfast", "Soup", "Salad", "Main", "Bakery"],
      recipes: [
            {
              tab: "Breakfast",
              url: "blinchiki.html",
  ...
  getters: {
    filteredRecipes(state) {
      return state.recipes.filter(r => r.name.toLowerCase().includes(state.searchQuery.toLowerCase()))
    }
  }
})
